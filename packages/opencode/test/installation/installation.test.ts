...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when @loveflow-ai/cli is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "@loveflow-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when @loveflow-ai/cli is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "@loveflow-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when @loveflow-ai/cli is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "@loveflow-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has @loveflow-ai/cli", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain(encodeURIComponent("@loveflow-ai/cli"))
    ...
    test("resolves version from GitHub releases redirect for curl method", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "curl" && args.includes("https://github.com/XiaomiMiMo/LoveFlow-Code/releases/latest"))
            return "HTTP/2 302\r\nlocation: https://github.com/XiaomiMiMo/LoveFlow-Code/releases/tag/v0.1.1\r\n"
          return ""
        },
      )
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package", async () => {
      ...
      expect(capturedArgs).toContain("@loveflow-ai/cli@2.0.0")
    })

...
const live: Layer.Layer<
  Service,
  never,
  Auth.Service | Config.Service | Provider.Service | Plugin.Service | Permission.Service | ActorRegistry.Service
> = Layer.effect(
  Service,
  Effect.gen(function* () {
    ...
    const buildSystemArray = Effect.fn("LLM.buildSystemArray")(function* (_input: {
      agent: Agent.Info
      model: Provider.Model
      system: string[]
      user: MessageV2.User
      ...
      // Local prompt-only mode: never add agent/provider, environment, skills,
      // instruction-file, memory, user.system, or plugin-transformed prompts.
      return []
    })

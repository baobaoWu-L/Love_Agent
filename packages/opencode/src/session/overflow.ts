...
export function usable(input: { cfg: Config.Info; model: Provider.Model }) {
  ...
  const reserved = Math.min(
    input.cfg.compaction?.reserved ?? COMPACTION_BUFFER,
    Math.floor(context * 0.15),
    ProviderTransform.maxOutputTokens(input.model),
  )
  const outputReserve = Math.min(ProviderTransform.maxOutputTokens(input.model), OUTPUT_CAP, Math.floor(context * 0.25))
